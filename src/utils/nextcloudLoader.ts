...
interface WebDavOptions {
    ...
    parser: (text: string, filePath: string, config: AstroConfig) => Promise<{ rendered?: RenderedContent, data: any }>,
...
export function nextcloudLoader(options: WebDavOptions): Loader {
    return {
        ...
        load: async (context) => {
            if (!ncHost) {
                throw new Error("No host provided. Make sure that all necessary environment variables are set.");
            }
            ...
            const files = await client.getDirectoryContents(ncPath, {deep: true}).catch(reason => {
                throw reason
            }).then(files => (files as FileStat[])
                    .filter(f => f.type === "file" && f.basename.endsWith(options.fileType)));
            ...
            const promises = files.map(async f => {
                ...
                text ??= await client.getFileContents(f.filename, {format: 'text'}).catch(reason => {
                    throw reason
                }) as string;
                
                const {rendered, data} = await options.parser(text, f.filename, context.config).catch(reason => {
                    throw reason
                });
...
export async function parseMarkdown(code: string, filePath: string, opts: AstroConfig) {
    ...
    markdownOptions.remarkPlugins.push(
    ...
    return {
        ...
        rendered: {
            html: render.code,
            metadata: {
                imagePaths: render.metadata.localImagePaths.concat(render.metadata.remoteImagePaths),
